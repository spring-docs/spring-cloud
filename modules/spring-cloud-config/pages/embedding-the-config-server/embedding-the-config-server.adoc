...
When setting `spring.cloud.config.server.bootstrap` to `true` you must also use a xref:spring-cloud-config-server:environment-repository.adoc#composite-environment-repositories[composite environment repository configuration].
For example
